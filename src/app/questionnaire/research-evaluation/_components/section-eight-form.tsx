...
import { updateStudentStatus } from "@/actions/update-student-status";
...
export function SectionEightnForm() {
    ...
    const { execute: saveAnswersAction, isPending: isSaving } = useAction(saveAnswers, {
        onSuccess: () => {
            // Após salvar as respostas, atualizar o status do estudante
    ...
    const { execute: updateStudentStatusAction, isPending: isUpdatingStatus } = useAction(updateStudentStatus, {
        onSuccess: () => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const result = await getQuestionsBySection(8);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex py-4">
                <Button
                    type="submit"
                    disabled={isSaving || isUpdatingStatus || questions.length === 0}
                    className="w-full"
                >
                    {isSaving ? "Salvando..." : isUpdatingStatus ? "Finalizando..." : "Finalizar"}
                </Button>
            </div>
        </form>
    );
